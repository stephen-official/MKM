...
// // const InventoryCard = ({ item, onAddToCart, cartQty, isRequestMode }) => {
// //   // Use 3 decimal places for precision logic
// //   const PRECISION = 3;
// //   const STEP = 0.001;
...
// //   // Helper to round strictly to 3 decimal places
...
// //       setOrderQty(0);
// //     } else if (!isRequestMode && val > maxAvailable) {
// //       setOrderQty(roundToPrecision(maxAvailable));
// //       toast.error(`Only ${maxAvailable} available`);
// //     } else {
// //       // If user is typing a decimal (ending in . or .0, etc.), keep raw string to avoid jumping
// //       // Otherwise, round the number to 3 decimal places
// //       if (rawValue.includes('.') && (rawValue.endsWith('0') || rawValue.endsWith('.'))) {
// //         setOrderQty(rawValue);
// //       } else {
// //         setOrderQty(roundToPrecision(val));
// //       }
// //     }
// //   };
...
// //       const current = parseFloat(prev) || 0;
// //       const newVal = Math.max(0, current + amount);
// //       const fixedNewVal = roundToPrecision(newVal);
...
// //         <div className={`flex items-center justify-between px-2 py-1 rounded-lg ${stockAvailable > 0 ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
// //           <span className="text-[7px] font-black uppercase">In Stock</span>
// //           <span className="text-[10px] font-black">{stockAvailable.toFixed(PRECISION)}</span>
// //         </div>
// //       </div>
...
// //             placeholder="0.000" 
// //             onChange={handleInputChange} 
// //             className="w-16 text-center bg-transparent text-[10px] font-black focus:outline-none appearance-none" 
// //           />
...
// //         <button 
// //           onClick={() => onAddToCart(item, parseFloat(orderQty) || 0)}
// //           className={`w-full py-2.5 rounded-xl font-black text-[8px] transition-all uppercase tracking-widest ${
// //             cartQty > 0 ? 'bg-blue-600 text-white shadow-blue-100' : 'bg-gray-900 text-white shadow-gray-200'
// //           } shadow-lg active:scale-95`}
// //         >
...
// useEffect(() => {
//   if (cartQty > 0) {
//     setOrderQty(parseFloat(cartQty));
//   } else {
//     setOrderQty("");
//   }
// }, [cartQty]);
...
//       const current = parseFloat(prev) || 0;
//       const newVal = Math.max(0, current + amount);
//       const fixedNewVal = Number(newVal.toFixed(3));
...
//         <button 
//           onClick={() => {
//   onAddToCart(item, parseFloat(orderQty) || 0);

//   if ((parseFloat(orderQty) || 0) === 0) {
//     setOrderQty("");
//   }
// }}
//           className={`w-full py-2.5 rounded-xl font-black text-[8px] transition-all uppercase tracking-widest ${
...
useEffect(() => {
  ...
  return (
    <div
  className={`bg-white rounded-3xl p-3 shadow-sm border border-gray-100 flex flex-col h-full transition-all ${
    isOutOfStock &&
    !isRequestMode &&
    !isDailyUse
  ...
  <div className="mb-3">
    ...
    >
      ...
      <span className="text-[10px] font-black">
        {stockAvailable.toFixed(PRECISION)}
      </span>
    </div>
  </div>
)}
